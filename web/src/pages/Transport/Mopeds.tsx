...
export default function MopedsPage() {
  ...
  const [mopeds, setMopeds] = useState<Vehicle[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedMoped, setSelectedMoped] = useState<Vehicle | null>(null);
  ...
  const loadMopeds = async () => {
    try {
      setLoading(true);
      const data = await getVehiclesByType('moped');
      setMopeds(data);
    } catch (err) {
      setError('Не вдалося завантажити мопеди');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleMopedClick = (moped: Vehicle) => {
    ...
    return (
      <div className="mopeds-page">
        ...
        <div className="error-container">
          <p>{error}</p>
          <button onClick={loadMopeds}>Спробувати знову</button>
        </div>
      </div>
    );
  }
      ...
      <section className="mopeds-catalog-section">
        <div className="mopeds-grid">
          {mopeds.map((moped) => (
            <div 
              key={moped.id} 
              className="moped-card"
              onClick={() => handleMopedClick(moped)}
              style={{ cursor: 'pointer' }}
            >
              <div className="moped-image">
                <img 
                  src={moped.photoUrl || '/image/Motobike.png'} 
                  alt={moped.displayName || `${moped.brand} ${moped.model}`} 
                />
              </div>

              <h3 className="moped-name">
                {moped.displayName || `${moped.brand} ${moped.model}`}
              </h3>
              ...
              <div className="moped-specs">
                <div className="spec-row">
                  <div className="spec-item">
                    ...
                    <span className="spec-text">{moped.batteryPct || 0}%</span>
                  </div>
                  ...
                  <div className="spec-item">
                    ...
                    <span className="spec-text">{moped.maxSpeed || 45} км/год</span>
                  </div>
                </div>

                <div className="spec-row">
                  <span className="spec-text spec-price">
                    {moped.unlockFee ? `${moped.unlockFee} ₴` : 'Ціна не вказана'}
                  </span>
                  <span className="spec-text spec-year">{moped.year || '2024'}</span>
                </div>
              </div>

              <button 
                className="moped-select-button"
                onClick={(e) => {
                  e.stopPropagation();
                  handleMopedClick(moped);
                }}
              >
        ...
        {mopeds.length === 0 && (
          <div className="no-mopeds">
            <p>Мопедів поки немає</p>
          </div>
        )}
      </section>
