...
export default function BikesPage() {
  ...
  const loadBikes = async () => {
    try {
      setLoading(true);
      const data = await getVehiclesByType('bike');
      setBikes(data);
    } catch (err) {
  ...
  const handleBikeClick = (bike: Vehicle) => {
    ...
    return (
      <div className="bikes-page">
        ...
        <div className="error-container">
          <p>{error}</p>
          <button onClick={loadBikes}>Спробувати знову</button>
        </div>
      </div>
    );
  }
          ...
          {bikes.map((bike) => (
            ...
            >
              <div className="bike-image">
                <img 
                  src={bike.photoUrl || '/image/Bicycle.png'} 
                  alt={bike.displayName || `${bike.brand} ${bike.model}`} 
                />
              </div>

              <h3 className="bike-name">
                {bike.displayName || `${bike.brand} ${bike.model}`}
              </h3>
              ...
              <div className="bike-specs">
                <div className="spec-row">
                  <div className="spec-item">
                    ...
                    <span className="spec-text">{bike.batteryPct || 0}%</span>
                  </div>
                  ...
                  <div className="spec-item">
                    ...
                    <span className="spec-text">{bike.maxSpeed || 25} км/год</span>
                  </div>
                </div>

                <div className="spec-row">
                  <span className="spec-text spec-price">
                    {bike.unlockFee ? `${bike.unlockFee} ₴` : 'Ціна не вказана'}
                  </span>
                  <span className="spec-text spec-year">{bike.year || '2024'}</span>
                </div>
              </div>

              <button 
                className="bike-select-button"
                onClick={(e) => {
                  e.stopPropagation();
